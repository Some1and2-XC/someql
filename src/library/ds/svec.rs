...
impl <T: PartialOrd + Clone> SVec<T> {
    ...
    /*
    pub fn new_random(len: i32, min: i32, max: i32) -> SVec<i32> {
...
impl <T: PartialOrd + Clone> From<Vec<T>> for SVec<T> {

    fn from(value: Vec<T>) -> Self {

        let mut out_arr = SVec::new();
...
impl <T: PartialOrd + Clone> Index<usize> for SVec<T> {

    type Output = T;

    /// Method for accessing a value at the specified index.
    /// Panics if the value is out of range or anything goes wrong with the access.
    fn index(&self, index: usize) -> &Self::Output {
        return self.data.get(index).expect(&format!("Can't access value at index: `{}`!", index));
    }
